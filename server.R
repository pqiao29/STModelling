...
shinyServer(function(input, output) {
  ...
  EstPlot <- function() {
    ...
    grid.arrange(arrangeGrob(gg_est, gg_good, ncol=1, nrow=2))
     
  }
  
  observeEvent(input$est,{
    output$beta_plot <- renderPlot({ EstPlot() }, height = 1250, width = 600)
  })
  
  output$downloadEst <- downloadHandler(
    filename = "Estimation.pdf",
    content = function(file) {
      ggsave(file, plot = EstPlot(), width = 12, height = 24)
    }
  )
  ...
  Selection <- function(){
    ...
    beta_plot <- data.frame("beta" = c(est), "row" = row_plot, "col" = col_plot)
      ...
      ggplot(data = beta_plot, aes(x = col, y = row)) +
  ...
  observeEvent(input$select,{
    ...
    output$sel_plot <- renderPlot({ Selection() }, height = 600, width = 600)
  ...
  output$downloadSel <- downloadHandler(
    ...
    content = function(file) {
      ggsave(file, plot = Selection(), width = 12, height = 12)
    }
  )
})